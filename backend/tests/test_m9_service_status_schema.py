...
"查服务状态"这一步从未真正采到证据，而任何日志/报告都看不出缺了它。
  ...
  M9-3 端到端：collect_rca_evidence 的采证结果包含 service.status
...
def test_m9_3_collect_includes_service_status() -> None:
    """M9-3: 端到端——collect_rca_evidence 采到 service.status 证据。"""
    from backend.app.agent.rca_drive import collect_rca_evidence

    engine = DefaultRCAEngine()
    called: list[str] = []

    def _fake_result(tool: CandidateTool) -> ToolResult:
        return ToolResult(
            tool=tool.name, args=tool.args, exit_code=0, stdout_truncated="ActiveState=failed"
        )

    gateway = mock.MagicMock()
    gateway.evaluate.return_value = mock.MagicMock(decision="allow", reason="")
    gateway.is_read_only.return_value = True

    async def _call(tool: CandidateTool, **_kw):
        called.append(tool.name)
        return CallOutcome(executed=True, result=_fake_result(tool))

    gateway.call = _call

    collected = asyncio.run(
        collect_rca_evidence(gateway, engine, "service failed", problem_type=_SCENARIO)
    )

    assert "service.status" in called, "M9-3: service.status 应真被调用"
