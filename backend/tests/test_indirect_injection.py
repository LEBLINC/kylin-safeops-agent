...
策略闸 deny 联合（本层不重测策略层逻辑；fixture 透传是真注入源的上游拦截）。
...
def test_fixture_indirect_injection_sample_generates_poison_note() -> None:
    ...
    真端点不返这个字段）。这是编排层 fixture 模拟"真 LLM 被日志投毒"的形态。
    """
    ...
    # ——这是编排层"模仿真 LLM 被投毒后 args 里出现危险 payload"的形态
...
def test_fixture_never_echoes_user_text_into_args() -> None:
    ...
    这是编排层 fixture 自身的"无回声"防护——上游不让危险 payload 透传到 args，
    下游策略闸无需拦"args 含 rm -rf /"的 attack surface。
    """
    dangerous_texts = [
...
# ---- 3. 间接注入的端到端 demo（fixture 模式 + 结果闸 result_gate）------------
...
def test_indirect_injection_finished_with_poisoned_tool_args_rejected() -> None:
    ...
    → 走策略闸（FILE001/command_injection_lure）→ REJECTED。

    本测试**只构造 fixture 输出** + **断言策略闸会拒**——不重测策略闸逻辑
    （策略层职责；test_policy_rules.py 已覆盖）。
    """
    ...
    # （策略层逻辑，由 test_policy_rules.py::test_FILE001_etc_shadow_deny 覆盖）
...
def test_fixture_returns_safe_intent_for_d10_text() -> None:
    ...
    本测试固化这一**特性边界**：fixture 模式 0 注入风险，可放心用于 CI；
    真端点模式必须走 input_gate（输入闸职责）。
    """
