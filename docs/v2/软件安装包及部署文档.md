...
本部署使用 `deploy/app/kylin-safeops-agent.service` 作为 app 单元（install.sh 安装此单元）：

| 单元 | 用途 | 本部署 |
|------|------|:---:|
| `deploy/app/kylin-safeops-agent.service` | app 后端主单元（8000）：`UMask=0077`、`EnvironmentFile=/etc/kylin-safeops/agent.env`、`Environment=KYLIN_LDAP_MOCK=false`、`ReadWritePaths` 覆盖审计库目录 | ✅ 采用 |
| `deploy/proxy/kylin-proxy.service` | 反代签名 sidecar（8080） | ✅ 采用 |

单元刻意**不设** `NoNewPrivileges`：app 需保留 setuid 能力经 sudo 调沙箱 wrapper；真正的 NNP 隔离在 wrapper 内层 `systemd-run`，施加到被执行的工具本身。

## 1.3 边界说明

`deploy/install.sh` 完成**后端代码部署 + Python 依赖 + systemd 单元安装 + env 骨架 + 前端静态文件拷贝**；Nginx 主配置片段与站点配置、沙箱 wrapper/sudoers 安装、TLS 证书、真实密钥填写为**人工步骤**（见 §7.3、§10、§11）。本文档逐步给出这些人工步骤，工程师照做即可完成完整部署。
...
| `/etc/kylin-safeops/agent.env` | app 侧环境变量（认证模式 / 共享密钥 / LDAP mock 开关 / 审计库路径） | 0600 | root:kylin-safeops |
...
制品摘要随代码变更而变，不在本文档内写死具体值（写死的摘要一旦落后于代码，
校验必然失败，反而让人误判"制品被篡改"）。发布打包时生成清单并随包分发：
...
sha256sum \
  ...
  deploy/verify.sh \
  deploy/nginx.conf \
  deploy/nginx-http-snippet.conf \
  deploy/app/kylin-safeops-agent.service \
  deploy/proxy/kylin-proxy.service \
  deploy/sandbox/kylin-safeops-run.sh \
  > SHA256SUMS
...
- 预期：每行以 `: OK` 结尾。
- 不符排查：某行报 `FAILED` → 该文件与发布时不一致，核对传输完整性与版本后再部署；报 `No such file` → 安装包不完整。
...
> **install.sh 的能力边界**：本脚本完成"后端代码 + Python 依赖 + systemd 单元 + env 骨架 + 前端静态文件"。Nginx 主配置片段、沙箱 wrapper/sudoers、TLS 证书、真实密钥填写为后续人工步骤（§7.3 ~ §11）。脚本执行成功 ≠ 部署验收完成，须完成 §17 验收清单。

## 7.3 反代 sidecar 目录布局

`install.sh` 把 `deploy/proxy` 与 `deploy/sso` 拷贝到 `/opt/kylin-safeops/deploy/` 下，与 sidecar 单元的 `ExecStart=... uvicorn deploy.proxy.proxy:app` 及 `proxy.py` 内部的 `from deploy.sso.ldap_client import ...` 保持同一模块路径。`deploy/` 目录无需 `__init__.py`——PEP 420 命名空间包可直接导入。

```bash
# 部署核对：确认模块可导入（应无输出、退出码 0）
sudo -u kylin-safeops /opt/kylin-safeops/.venv/bin/python \
  -c "import sys; sys.path.insert(0,'/opt/kylin-safeops'); import deploy.proxy.proxy, deploy.sso.ldap_client"
```

- 预期：无输出，退出码 0。
- 不符排查：`ModuleNotFoundError: No module named 'deploy'` → 确认 `/opt/kylin-safeops/deploy/proxy/proxy.py` 与 `/opt/kylin-safeops/deploy/sso/ldap_client.py` 均存在（目录名须为 `deploy/proxy`、`deploy/sso`，不是 `deploy_proxy`、`deploy_sso`）。
...
**app 侧**（/etc/kylin-safeops/agent.env + systemd 单元 Environment）：
...
`deploy/nginx.conf` 用 `listen 443 ssl http2;` 合并语法，**1.24 与 1.25+ 均可直接使用，无需按版本改文件**：

```bash
nginx -v    # 记录版本备查即可
```

- 麒麟 V11 随附 nginx 1.24：只认合并语法；独立指令 `http2 on;` 是 1.25.1+ 才引入的，在 1.24 上会 `unknown directive` 直接启动失败。
- nginx 1.25+：合并语法仍受支持（仅产生弃用告警，不影响启动）。
...
| nginx -t 报 `unknown limit_req_zone "kylin_api"` | 主配置缺 http{} 片段 | 按 §11.2 放置 `deploy/nginx-http-snippet.conf` |
| nginx -t 报 `could not build server_names_hash` | bucket size 默认 32 偏小 | 同上片段含 `server_names_hash_bucket_size 64;` |
...
sudo cp -a /etc/kylin/proxy.env /etc/kylin-safeops/agent.env /var/backups/kylin-safeops/
...
sudo rm -f /etc/systemd/system/kylin-safeops-agent.service /etc/systemd/system/kylin-proxy.service
...
| 目标机 nginx 主配置 | 需含 limit_req_zone 与 bucket size | 放置 `deploy/nginx-http-snippet.conf`（§11.2） |
...
| `KYLIN_SSE_QUEUE_MAX` | `512` | 按需 | | 单条 trace 的 SSE 事件队列深度上限；满则 fail-fast 而非无界堆积 |
| `KYLIN_SSE_MAX_CONN` | `100` | 按需 | | SSE **并发连接**上限（chat 流 / probe 流 / readiness 判定三处共用） |
...
- `deploy/install.sh`、`deploy/verify.sh`、`deploy/app/kylin-safeops-agent.service`、`deploy/proxy/kylin-proxy.service`、`deploy/nginx.conf`、`deploy/nginx-http-snippet.conf`、`deploy/sandbox/kylin-safeops-run.sh`、`deploy/sandbox/kylin-safeops-sudoers`、`deploy/sso/ldap_client.py`
